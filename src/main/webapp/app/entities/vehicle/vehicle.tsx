  ...
  return (
    <div>
      <h2 id="vehicle-heading" data-cy="VehicleHeading">
        <Translate contentKey="jhipsterSampleApplication1App.vehicle.home.title">Vehicles</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="jhipsterSampleApplication1App.vehicle.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/vehicle/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="jhipsterSampleApplication1App.vehicle.home.createLabel">Create new Vehicle</Translate>
          </Link>
        </div>
      </h2>
        ...
        {vehicleList && vehicleList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="jhipsterSampleApplication1App.vehicle.id">ID</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('plate')}>
                  <Translate contentKey="jhipsterSampleApplication1App.vehicle.plate">Plate</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('plate')} />
                </th>
                <th className="hand" onClick={sort('enabled')}>
                  <Translate contentKey="jhipsterSampleApplication1App.vehicle.enabled">Enabled</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('enabled')} />
                </th>
                <th className="hand" onClick={sort('type')}>
                  <Translate contentKey="jhipsterSampleApplication1App.vehicle.type">Type</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('type')} />
                </th>
                <th className="hand" onClick={sort('capacity')}>
                  <Translate contentKey="jhipsterSampleApplication1App.vehicle.capacity">Capacity</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('capacity')} />
                </th>
                <th className="hand" onClick={sort('color')}>
                  <Translate contentKey="jhipsterSampleApplication1App.vehicle.color">Color</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('color')} />
                </th>
                <th className="hand" onClick={sort('createdDate')}>
                  <Translate contentKey="jhipsterSampleApplication1App.vehicle.createdDate">Created Date</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('createdDate')} />
                </th>
                <th className="hand" onClick={sort('defaultImage')}>
                  <Translate contentKey="jhipsterSampleApplication1App.vehicle.defaultImage">Default Image</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('defaultImage')} />
                </th>
                <th className="hand" onClick={sort('defaultImageData')}>
                  <Translate contentKey="jhipsterSampleApplication1App.vehicle.defaultImageData">Default Image Data</Translate>{' '}
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="jhipsterSampleApplication1App.vehicle.home.notFound">No Vehicles found</Translate>
            </div>
          )
        )}
      </div>
