  ...
  return (
    <div>
      <h2 id="destination-heading" data-cy="DestinationHeading">
        <Translate contentKey="jhipsterSampleApplication1App.destination.home.title">Destinations</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="jhipsterSampleApplication1App.destination.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/destination/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="jhipsterSampleApplication1App.destination.home.createLabel">Create new Destination</Translate>
          </Link>
        </div>
      </h2>
        ...
        {destinationList && destinationList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="jhipsterSampleApplication1App.destination.id">ID</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('code')}>
                  <Translate contentKey="jhipsterSampleApplication1App.destination.code">Code</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('code')} />
                </th>
                <th className="hand" onClick={sort('enabled')}>
                  <Translate contentKey="jhipsterSampleApplication1App.destination.enabled">Enabled</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('enabled')} />
                </th>
                <th className="hand" onClick={sort('defaultImage')}>
                  <Translate contentKey="jhipsterSampleApplication1App.destination.defaultImage">Default Image</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('defaultImage')} />
                </th>
                <th className="hand" onClick={sort('defaultImageData')}>
                  <Translate contentKey="jhipsterSampleApplication1App.destination.defaultImageData">Default Image Data</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('defaultImageData')} />
                </th>
                <th className="hand" onClick={sort('cssStyle')}>
                  <Translate contentKey="jhipsterSampleApplication1App.destination.cssStyle">Css Style</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('cssStyle')} />
                </th>
                <th className="hand" onClick={sort('createdDate')}>
                  <Translate contentKey="jhipsterSampleApplication1App.destination.createdDate">Created Date</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('createdDate')} />
                </th>
                <th>
                  <Translate contentKey="jhipsterSampleApplication1App.destination.createdBy">Created By</Translate>{' '}
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="jhipsterSampleApplication1App.destination.home.notFound">No Destinations found</Translate>
            </div>
          )
        )}
      </div>
